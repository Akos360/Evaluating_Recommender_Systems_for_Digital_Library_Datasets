...
%With the exponential growth of digital content, users increasingly struggle to identify relevant information—especially within digital libraries containing thousands of texts. Recommender systems address this challenge by delivering personalized content suggestions. This paper evaluates several content-based algorithms—including TF-IDF, LSA, GloVe, FastText, BoW, and BERT—for recommending books and paragraphs based on textual content.
%
%Two datasets were used: full paragraphs and book descriptions. The models were compared using metrics such as average similarity, diversity, coverage, confidence, execution time, and memory usage. FastText and GloVe achieved the highest recommendation relevance, with average similarity scores of 0.945 and 0.981 on paragraph-level data, and 0.939 and 0.989 respectively on description-level data. However, FastText had significantly higher memory usage and runtime. BERT, while more computationally expensive, offered high confidence (0.95+) with moderate similarity. Simpler models like BoW and TF-IDF were faster and lighter on resources but showed lower accuracy and coverage.
%
%These findings highlight trade-offs between model accuracy, computational efficiency, and recommendation diversity, providing actionable guidance for digital library system designers. The results also suggest that combining embeddings like GloVe or BERT with lightweight retrieval techniques could lead to more scalable and effective hybrid systems.
%
%
%
%With the exponential growth of digital content, users increasingly struggle to identify relevant information, particularly within digital libraries. Recommender systems alleviate this by generating personalized suggestions based on user behavior or item features. This paper evaluates multiple content-based algorithms—including TF-IDF, LSA, GloVe, FastText, and BERT—on their ability to generate paragraph and book-level recommendations from a library dataset. We benchmark each model using metrics such as similarity, confidence, diversity, and coverage, along with computational performance indicators like runtime and memory usage. Our findings show that while transformer-based and neural embeddings (e.g., FastText, BERT) achieve higher relevance, traditional methods like LSA strike a better balance between accuracy, diversity, and efficiency. These results provide actionable insights into algorithm selection for scalable recommendation systems in academic libraries.
...
%By benchmarking these factors, we highlight the trade-offs between accuracy and computational efficiency, offering insights into the strengths and weaknesses of each algorithm for book recommendation tasks in digital libraries.
...
%\begin{figure}[!htbp]
%\centering
%\includegraphics[width=0.5\textwidth]{img/collaborative_example.png}
...
%\begin{figure}
%\centering
%\includegraphics[width=0.5\textwidth]{img/knowledge_graph_example.png}
%%\includegraphics[width=\textwidth]{fig1.eps}
...
%\textbf{Embedding and Recommendation Process}\\
%Each algorithm followed a multi-step process to generate recommendations. First, all paragraphs or descriptions were transformed into vector representations specific to the chosen model. For example, TF-IDF and BoW tokenized the input by removing stop words and then constructed sparse matrices based on term frequency (with a vocabulary size limited to 5000 for TF-IDF). LSA reduced these TF-IDF representations using Truncated Singular Value Decomposition with 500 latent dimensions. GloVe and FastText used pretrained or self-trained word embeddings, computing paragraph vectors as the mean of word vectors found in the text. FastText further captured subword information through character n-grams, and BERT (specifically, the all-MiniLM-L6-v2 variant) produced contextual embeddings by encoding entire sentences using transformer attention mechanisms.
...
% SMALL TABLE
%\begin{table}
%\caption{Detailed Results - Using Book Descriptions}\label{tab:descriptions_full}
%\centering
%\resizebox{\textwidth}{!}{
%\begin{tabular}{|l|l|l|l|l|l|l|l|}
%\hline
%\textbf{Algorithm} & \textbf{Avg Similarity} & \textbf{Coverage (\%)} & \textbf{Confidence} & \textbf{Diversity} & \textbf{Elapsed Time (s)} & \textbf{Memory (MB)} & \textbf{CPU Time (s)} \\
%\hline
%TF-IDF    & 0.196 & 0.34  & 0.95279 & 0.0027029 & 3.973  & 293.49  & 1.729  \\ \hline
%BoW       & 0.254 & 0.23  & 0.93772 & 0.0049934 & 15.256 & 378.45  & 0.809  \\ \hline
%FastText  & 0.945 & 0.53  & 0.99023 & 0.0000988 & 49.959 & 2002.80 & 44.964 \\ \hline
%GloVe     & 0.981 & 0.25  & 0.99787 & 0.0000045 & 14.888 & 420.78  & 10.314 \\ \hline
%LSA       & 0.425 & 0.12  & 0.93325 & 0.0055086 & 13.248 & 744.64  & 8.179  \\ \hline
%BERT      & 0.359 & 0.20  & 0.94778 & 0.0033437 & 74.456 & 1227.44 & 103.399\\ \hline
%\end{tabular}
%}
%\end{table}
...
%
%To evaluate each algorithm, we calculated several key metrics based on the similarity scores of the recommended items. \textbf{Average Similarity} measures how close the recommended items are to the input, based on cosine similarity. \textbf{Item Coverage} reflects how many unique items the algorithm was able to recommend from the entire dataset, indicating how widely it explores the content. \textbf{Confidence} is calculated as the inverse of the standard deviation of similarity scores, meaning that if the scores are consistent, the confidence is higher. \textbf{Diversity} shows how varied the recommendations are by measuring the variance in similarity scores—a higher value suggests the algorithm returns a more diverse set of results. These metrics were selected because they provide useful insight into the behavior and quality of content-based recommendation models, especially in the absence of user interaction data. Since no explicit or implicit user feedback was available for the dataset, traditional recommendation metrics like Precision, Recall, or NDCG could not be applied. Instead, we focused on metrics that assess the internal structure and output characteristics of the algorithms using only the content and similarity scores.
...
%\end{thebibliography}
 ...
 \vspace{-1em} % tighten space before references if needed
\begin{multicols}{2}
